module Cryptopals
  module CtrSubstitutions
    ...
    def statistical_attack
      ...
      ciphertexts = SAMPLES2.map { |s| aes_128_ctr(Base64.decode64(s), key) }
      size = ciphertexts.map(&:size).min
      cipherbytes = ciphertexts.map { |s| s.bytes.take(size) }
      key = probable_keys(cipherbytes.flatten, [size]).first
      decrypt(cipherbytes, key.bytes)
    end
